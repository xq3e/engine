...
extern qboolean X_IsPK3PureCompatible(const char* mode, const char* basename);
...
static qboolean FS_PakIsPure( const pack_t *pack ) {
	...
	if ( fs_numServerPaks ) {
		...
		if (X_IsPK3PureCompatible(pack->pakGamename, pack->pakBasename))
			return qtrue;
...
#ifndef DEDICATED
				if (X_IsPK3PureCompatible(search->pack->pakGamename, search->pack->pakBasename))
					continue;
#endif
