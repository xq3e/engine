...
#ifndef DEDICATED
extern qboolean xmod_disable_output;
#endif
...
static void QDECL Com_PrintfInternal(int section, int len, char* msg);
...
void QDECL Com_Printf(const char* fmt, ...)
{
	...
	Com_PrintfInternal(0, len, msg);
}

void QDECL Com_Printf2(const char* fmt, ...)
{
	va_list		argptr;
	char		msg[MAXPRINTMSG];
	int			len;
...
//	Swap_Init ();
...
#ifndef DEDICATED
	// xmod initialization before any config
	X_InitXMod();
#endif
  ...
  // get dedicated here for proper hunk megs initialization
	...
	//
#ifndef DEDICATED
	com_maxfps = Cvar_Get( "com_maxfps", "125", CVAR_ARCHIVE); // try to force that in some light way
