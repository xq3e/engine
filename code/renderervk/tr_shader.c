...
static qboolean EqualTCgen( int bundle, const shaderStage_t *st1, const shaderStage_t *st2 )
{
	...
	for ( tm = 0; tm < b1->numTexMods; tm++ ) {
		...
		if (tm1->type == TMOD_ROTATE2 && tm1->rotateSpeed != tm2->rotateSpeed) {
			return qfalse;
		}
	}
...
void R_UpdateShaderColorByHandle(qhandle_t hShader, vec3_t color)
{
	...
	shader->stages[0]->rgbGen = CGEN_CONST;
	shader->stages[0]->constantColor[0] = color[0] * 255;
	shader->stages[0]->constantColor[1] = color[1] * 255;
	shader->stages[0]->constantColor[2] = color[2] * 255;
	shader->stages[0]->constantColor[3] = 255;
}
