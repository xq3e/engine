...
#define _DrawStr(xx, yy, sz, alp, fl, chrmap, str) \
			{\
				vec4_t color; \
...
#define DrawBarStr(bar, xx, yy, sz, alp, str) \
			 _DrawStr((bar)->x + ((bar)->scale * (xx)), (bar)->y + ((bar)->scale * (yy)), (bar)->scale * (sz), alp, StringWithShadow, xmod.rs.shaderXCharmap, str)

#define DrawBarStrInCenter(bar, xx, yy, sz, alp, str) \
			_DrawStr((bar)->x + ((bar)->scale * (xx)), (bar)->y + ((bar)->scale * (yy)), (bar)->scale * (sz), alp, StringWithShadow|StringInCenter, xmod.rs.shaderXCharmap, str)

#define DrawBarStrRightAligned(bar, xx, yy, sz, alp, str) \
			_DrawStr((bar)->x + ((bar)->scale * (xx)), (bar)->y + ((bar)->scale * (yy)), (bar)->scale * (sz), alp, StringWithShadow|StringRightAligned, xmod.rs.shaderXCharmap, str)

#define DrawFatBarStr(bar, xx, yy, sz, alp, str) \
			_DrawStr((bar)->x + ((bar)->scale * (xx)), (bar)->y + ((bar)->scale * (yy)), (bar)->scale * (sz), alp, StringWithShadow, xmod.rs.shaderCharmap[2], str)

#define DrawFatBarStrInCenter(bar, xx, yy, sz, alp, str) \
			_DrawStr((bar)->x + ((bar)->scale * (xx)), (bar)->y + ((bar)->scale * (yy)), (bar)->scale * (sz), alp, StringWithShadow|StringInCenter, xmod.rs.shaderCharmap[2], str)

#define DrawFatBarStrRightAligned(bar, xx, yy, sz, alp, str) \
			_DrawStr((bar)->x + ((bar)->scale * (xx)), (bar)->y + ((bar)->scale * (yy)), (bar)->scale * (sz), alp, StringWithShadow|StringRightAligned, xmod.rs.shaderCharmap[2], str)

static void ShowScoreTable(void);
static void HideScoreTable(void);
static void RedirectScoreCommandToCgame();

static void DrawScoreTable();
static void CheckAndTurnOfAutoshow();

static void DrawTopHud();

static void DrawFFAScoreTable();
static void Draw1v1ScoreTable();
static void DrawTeamScoreTable();
...
static void DrawTopHud()
{
	...
	CalculateCenterBar(&bar, 640.0, 480.0);
...
static const char* GetCurrentModeLimits()
{
	static char buffer[256];
...
static void DrawVoteBar(Bar* bar, float height)
{
	...
	int yes = atoi(X_GetConfigString(CS_VOTE_YES));
