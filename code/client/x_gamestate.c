...
static void UpdateRoundStartTime();
static void UpdateWarmupTime();
...
static void LoadModelIcons(XPlayerState* ps, char* model);

static unsigned short CalculatePlayerSignature(char* cmd);
static void UpdateXModeConfiguartion();
static void ParseXModCommands(const char* cmdset);
static void UpdatePlayerPositionOnFakeSound(int clientNum, vec3_t origin);
...
static void RemoveExpiredEventsFromCache();
...
static void ParsePlayerInfo(XPlayerState* ps, char* info)
{
	...
	char* model = Info_ValueForKey(info, "model");
	...
	char* lvl = Info_ValueForKey(info, "skill");
	if (strlen(lvl))
	{
		ps->isbot = qtrue;
		ps->botlvl = atoi(lvl);
...
static unsigned short CalculatePlayerSignature(char* cmd)
{
	...
	char* data = Info_ValueForKey(cmd, "c1");
	CRC_ContinueProcessString(&sign, data, strlen(data));

	data = Info_ValueForKey(cmd, "c2");
	CRC_ContinueProcessString(&sign, data, strlen(data));

	data = Info_ValueForKey(cmd, "model");

	char* skin = strchr(data, '/');
	if (skin)
		*skin = '\0';

	CRC_ContinueProcessString(&sign, data, strlen(data));

	data = Info_ValueForKey(cmd, "hmodel");
...
qboolean X_GS_UpdatePlayerXStats1()
{
...
static void UpdatePlayerPositionOnFakeSound(int entityNum, vec3_t origin)
{
