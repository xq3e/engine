...
static char X_HELP_HCK_PS_ENEMY_HITBOX_2D[] = "\n ^fx_hck_ps_enemy_hitbox_2d^5 0|...|1^7\n\n"
											  "   Draw 2D-hitbox for enemy players.\n";
...
static void DrawPlayerHitbox(PlayerModel model, refEntity_t* ref)
{
	 ...
	 || xmod.snap.ps.persistant[PERS_TEAM] != xmod.gs.ps[client].team)
	{
		...
		else if (x_hck_ps_enemy_hitbox->integer == 2)
		{
			...
			entity.radius = 128;
			entity.shaderRGBA[0] = 0;
			entity.shaderRGBA[1] = 255;
			entity.shaderRGBA[2] = 0;
...
static void MakeDeadBodyBlack(PlayerModel model, refEntity_t* ref)
{
	...
	{
		ref->shaderRGBA[0] = ref->shaderRGBA[1] = ref->shaderRGBA[2] = 220 - (55 * x_ps_dead_body_black->integer);
