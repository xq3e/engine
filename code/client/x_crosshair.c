...
static void TracePlayerOnAim(vec3_t origin, float distance, vec3_t axis);
static void ValidateAndSetAimedTarget(int client, float distance, vec3_t source, vec3_t target);
...
static float GetPulseMultiplier()
{
	HitCrosshairPulse* hp = &xmod.ch.hp;
...
static float GetPulseOpaqueMultiplier()
{
	HitCrosshairPulse* hp = &xmod.ch.hp;
...
static float CalculateCrosshairScale()
{
	float scale = 1.0f;
