...
static void LoadXModeResources();
static void PrintWelcomeLogo();
...
static void PrintWelcomeLogo()
{
	...
	char* messages[] = {
		...
		"^7\n"
		"^7\n\n"
		"^7   Belarus without a dictator, stop tarakan!\n\n"
		"^7       From ^1XQ3E^7 with love, have fun ;)\n\n",
...
static PrintCommandState(char* key, qboolean deflt)
{
...
static void Print_State(void)
{
	qboolean xmod = IsXModeActive();
	qboolean xhck = X_HckActive();
	...
	while (info)
	{
		...
		if (!Q_stricmp(key, "x_enable"))
			PrintCommandState(key, xmod);
		else
			PrintCommandState(key, xhck);
	}

	Com_Printf("\n");
}

static char* UntokenizeNickname(char* nickname, char* output, int size)
{
	int b = 0;
