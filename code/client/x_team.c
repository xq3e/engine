...
static char X_HELP_TEAM_FOEHACK[]    = "\n ^fx_hck_team_foehack^5 0|1^7\n\n"
									   "   Make team icon (triangle) visible through walls\n";
...
{
	const char* name = R_GetModelNameByHandle(model);
...
void X_Team_CustomizeFreezeEffect(refEntity_t* ref)
{
	...
	if (xmod.snap.ps.persistant[PERS_TEAM] == TEAM_SPECTATOR)
	{
		...
		R_UpdateShaderColorByHandle(ref->customShader, color);
	}
